...
TEST(Point, Geographic2Ecef) {
  auto longlat = Point(0, 0);
  auto geocent = static_cast<EarthCenteredEarthFixed>(longlat);
  EXPECT_EQ(6378137.0, geocent.x());
  EXPECT_EQ(0.0, geocent.y());
  EXPECT_EQ(0.0, geocent.z());

  longlat = Point(78.042068, 27.173891);
  geocent = static_cast<EarthCenteredEarthFixed>(longlat);
