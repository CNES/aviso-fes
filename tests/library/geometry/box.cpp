...
TEST_F(BoxTest, CopyConstructor) {
  geometry::Box original{p1, p2};
  geometry::Box copy{original};

  EXPECT_DOUBLE_EQ(copy.min_corner().lon(), original.min_corner().lon());
  EXPECT_DOUBLE_EQ(copy.min_corner().lat(), original.min_corner().lat());
  EXPECT_DOUBLE_EQ(copy.max_corner().lon(), original.max_corner().lon());
  EXPECT_DOUBLE_EQ(copy.max_corner().lat(), original.max_corner().lat());
}

// Test copy constructor with dateline crossing box
TEST_F(BoxTest, CopyConstructorDatelineCrossing) {
  geometry::Box original{p_dateline1, p_dateline2};
  geometry::Box copy{original};
...
TEST_F(BoxTest, CopyAssignmentOperator) {
  geometry::Box original{p1, p2};
  geometry::Box assigned;

  assigned = original;

  EXPECT_DOUBLE_EQ(assigned.min_corner().lon(), original.min_corner().lon());
  EXPECT_DOUBLE_EQ(assigned.min_corner().lat(), original.min_corner().lat());
  EXPECT_DOUBLE_EQ(assigned.max_corner().lon(), original.max_corner().lon());
  EXPECT_DOUBLE_EQ(assigned.max_corner().lat(), original.max_corner().lat());
}

// Test self-assignment
TEST_F(BoxTest, SelfAssignment) {
  geometry::Box box{p1, p2};

  box = box;  // Self-assignment
...
TEST_F(BoxTest, MoveConstructor) {
  geometry::Box original{p1, p2};
  geometry::Point orig_min = original.min_corner();
  geometry::Point orig_max = original.max_corner();

  geometry::Box moved{std::move(original)};

  EXPECT_DOUBLE_EQ(moved.min_corner().lon(), orig_min.lon());
  EXPECT_DOUBLE_EQ(moved.min_corner().lat(), orig_min.lat());
  EXPECT_DOUBLE_EQ(moved.max_corner().lon(), orig_max.lon());
  EXPECT_DOUBLE_EQ(moved.max_corner().lat(), orig_max.lat());
}

// Test move assignment operator
TEST_F(BoxTest, MoveAssignmentOperator) {
  geometry::Box original{p1, p2};
  geometry::Point orig_min = original.min_corner();
  geometry::Point orig_max = original.max_corner();
  ...
  EXPECT_DOUBLE_EQ(assigned.min_corner().lon(), orig_min.lon());
  EXPECT_DOUBLE_EQ(assigned.min_corner().lat(), orig_min.lat());
  EXPECT_DOUBLE_EQ(assigned.max_corner().lon(), orig_max.lon());
  EXPECT_DOUBLE_EQ(assigned.max_corner().lat(), orig_max.lat());
}

// Test setter methods
TEST_F(BoxTest, SetMinCorner) {
  geometry::Box box{p1, p2};
  geometry::Point new_min{5.0, 15.0};

  box.set_min_corner(new_min);

  EXPECT_DOUBLE_EQ(box.min_corner().lon(), 5.0);
  EXPECT_DOUBLE_EQ(box.min_corner().lat(), 15.0);
  EXPECT_DOUBLE_EQ(box.max_corner().lon(), 30.0);
  EXPECT_DOUBLE_EQ(box.max_corner().lat(), 40.0);
}

TEST_F(BoxTest, SetMaxCorner) {
  geometry::Box box{p1, p2};
  geometry::Point new_max{35.0, 45.0};

  box.set_max_corner(new_max);

  EXPECT_DOUBLE_EQ(box.min_corner().lon(), 10.0);
  EXPECT_DOUBLE_EQ(box.min_corner().lat(), 20.0);
  EXPECT_DOUBLE_EQ(box.max_corner().lon(), 35.0);
  EXPECT_DOUBLE_EQ(box.max_corner().lat(), 45.0);
}

// Test setting corners that would cause dateline crossing
TEST_F(BoxTest, SetCornersDatelineCrossing) {
  geometry::Box box{p1, p2};

  box.set_min_corner(p_dateline1);
  box.set_max_corner(p_dateline2);
...
// Test boost geometry integration
TEST_F(BoxTest, BoostGeometryIntegration) {
  geometry::Box box{p1, p2};

  // Test that boost geometry algorithms work with our box
  double area = boost::geometry::area(box);
  EXPECT_GT(area, 0.0);

  // Test centroid calculation
  geometry::Point centroid;
  boost::geometry::centroid(box, centroid);
  EXPECT_GT(centroid.lon(), box.min_corner().lon());
  EXPECT_LT(centroid.lon(), box.max_corner().lon());
  EXPECT_GT(centroid.lat(), box.min_corner().lat());
  EXPECT_LT(centroid.lat(), box.max_corner().lat());
}
