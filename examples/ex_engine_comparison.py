...
This example demonstrates the differences between PyFES's two prediction engines:
...
if darwin_only:
    print(f"\nExample Darwin-only constituents: {', '.join(list(darwin_only)[:5])}")
if perth_only:
    print(f"Example PERTH5-only constituents: {', '.join(list(perth_only)[:5])}")
